...
function saveResponse(req, cachedObj, callback) {
	...
	if ( req.proxy.data ) {
		var encoding = 'utf8Data' ;
		if ( req.proxy.headers['content-type'] && req.proxy.headers['content-type'].match(/^image/) ) // not all responses have a 'content-type' (like a 302 response)
			encoding = 'binaryData' ;
